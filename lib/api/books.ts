...
function transformBook(book: Book): Book {
  return {
    ...
    id: String(book.id),
    // Ensure book_type is set (default to textbook for legacy data)
...
export const booksApi = {
  ...
  create: async (data: BookFormData): Promise<Book> => {
    ...
    // Note: book_id is now auto-generated by backend based on book_type
    const backendData = {
  ...
  update: async (id: string, data: Partial<BookFormData>): Promise<Book> => {
    // Transform frontend field names to backend field names
    // Note: book_type cannot be changed after creation
